...
static double SigDistance(double lat1, double lon1, double lat2, double lon2)
{
...
int PutSignal(double lat, double lon, unsigned char signal, FILE *fd, int tx, double txlat, double txlon, unsigned char signal1)
{
	/* This function writes a signal level (0-255)
	   at the specified location for later recall. */

	int	x, y, x0, y0, x1, y1, indx;
	char	found;
	...
	if (found)
	{
		x0 = (int)rint(ppd * (dem[indx].min_north - min_north));
		x1 = (int)rint(ppd * (max_north - min_north));
		y0 = ippd - (int)rint(ppd * (max_west - dem[indx].max_west));
		y1 = (int)rint(ppd * (max_west - min_west));
		if (fd != NULL) fprintf(fd,"%d %d %d %lf\n",y1 - (y + y0),x1 - (x + x0),signal1,SigDistance(txlat,txlon,lat,lon));
...
void PlotLRPath(struct site source, struct site destination, unsigned char mask_value, FILE *fd, int txn)
{
		...
		if ((GetMask(path.lat[y],path.lon[y])&248)!=(mask_value<<3))
		{
			...
			if (LR.erp!=0.0)
			{
				if (dbm)
				{
					...
					iifs = (unsigned char)ifs;

					if (ofs>ifs)
						ifs=ofs;

					PutSignal(path.lat[y],path.lon[y],(unsigned char)ifs,fd,txn,source.lat,source.lon,iifs);
				}
...
void PlotLRMap(struct site source, double altitude, char *plo_filename, int txn)
{
	/* This function performs a 360 degree sweep around the
	   transmitter site (source location), and plots the
	...
	if (dodsf)
	{
		snprintf(dsf_filename,sizeof(dsf_filename) - 1,"%s.dsf",source.name);
		fd = fopen(dsf_filename,"w");
	}
	else
	{
		if (plo_filename[0]!=0)
	...
	if (fd != NULL)
	{
		if (dodsf)
		{
			...
			for (indx=0, found=0; indx<MAXPAGES && found==0;)
			{
				x = (int)rint(ppd*(source.lat-dem[indx].min_north));
				y = mpi - (int)rint(ppd*(LonDiff(dem[indx].max_west,source.lon)));
			...
			if (found)
			{
				...
				y0 = ippd - (int)rint(ppd * (max_west - dem[indx].max_west));
				y1 = (int)rint(ppd * (max_west - min_west));
				fprintf(fd,"%s %f %f %d %d %d %d %d %d %d %d\n",source.name,source.lat,source.lon,y1 - (y + y0),
					x1 - (x + x0),y1,x1,min_west, max_west, min_north, max_north);
			}
		}
		else
		{
	...
	for (lon=minwest, x=0, y=0; (LonDiff(lon,(double)max_west)<=0.0); y++, lon=minwest+(dpp*(double)y))
	{
		...
		PlotLRPath(source,edge,mask_value,fd,txn);
		count++;
		...
		while (feof(fd)==0)
		{
			if (LR.erp==0.0)
			{
				...
				if (contour_threshold==0 || (fabs(ano)<=(double)contour_threshold))
				{
					...
					PutSignal(latitude,longitude,((unsigned char)round(ano)),NULL,0,0.0,0.0,0);
				}
			}
...
int main(int argc, char *argv[])
{
	...
	if (HD_MODE==1)
		strncpy(splat_name,"SPLAT-s! HD\0",10);
	else
		strncpy(splat_name,"SPLAT-s!\0",7);
	...
	if (argc==1)
	{
		...
		fprintf(stdout,"     -dsf generate a dsf file for each tx site\n");
		...
		for (x=0; x<txsites && x<max_txsites; x++)
		{
			if (coverage)
				PlotLOSMap(tx_site[x],altitude);
					...
					PlotLRMap(tx_site[x],altitudeLR,ano_filename,x);
