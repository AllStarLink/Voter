...
#define	ASSERT_ENC_CS_IO {ENC_CS_IO=0; SSP2CON1bits.SSPEN = 1; }
#define	DEASSERT_ENC_CS_IO ENC_CS_IO=1;
...
//}
	...
	#define DUMPMACREG(x) {BankSel(x); c = ReadMACReg(x & 0xff); printf("MACREG %s is %02x hex\n",#x,c.Val); } 
	#define DUMPPHYREG(x) {printf("PHYREG %s is %04x hex\n",#x,ReadPHYReg(x).Val); } 
	
	void DumpETHReg(void)
	{
		REG c;
