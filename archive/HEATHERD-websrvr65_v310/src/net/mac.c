...
/* SBC65EC and SBC68EC - use port E*/
...
#if defined(BRD_SBC44EC)
    //Uses bits PORT B0 to B4.
    //Parameter a will always have a value from 0-31, upper 3 bits of a will never be set
    #ifdef NIC_ENABLE_B6B7
    #define WRITE_NIC_ADDR(a)   (NIC_ADDR_IO = (a | (NIC_ADDR_IO & 0xc0)) )
    #else
    #define WRITE_NIC_ADDR(a)   (NIC_ADDR_IO = a)
    #endif
#elif defined(BRD_SBC45EC)
    //Uses bits PORT A0, A1, A2, A3 and A5.
    //Parameter a will always have a value from 0-63, upper 2 bits of a will never be set!
    #define WRITE_NIC_ADDR(a)   (NIC_ADDR_IO = a)
#elif (defined(BRD_SBC65EC) || defined(BRD_SBC68EC))
...
void    MACInit(void)
{
    ...
    //if ( (NICGet(ISR) & 0x80) != 0 )
    {
        ...
        //MACInitEepromRegs();  //Only required for old Rev 1 boards
        #endif
...
void MACInitEepromRegs(void) {
        ...
        //Because the EEPROM read pin is floating on some boards (only old rev 1 boards), the registers
        //initialized by the EEPROM will all contain non defined values. Write them with correct values
        NICPut(RTL9346CR, 0xc0);    //Config register write enable
