...
void busTask(void) {
    ...
    //Currently UDP 1 can only be linked to Serial 1 or 2
    if ((busId >= BUSID_SER1) && (busId <= BUSID_SER2) && (udpBus1 != INVALID_UDP_SOCKET)) {
    ...
    //Currently UDP 1 can only be linked to Serial 1 or 2
    if ((busId >= BUSID_SER1) && (busId <= BUSID_SER2) && (udpBus2 != INVALID_UDP_SOCKET)) {
