...
/* Buffer size
 * Noise History buffer size. Must be a power of 2.
 */
#define NHBSIZE		16		
 ...
 * 16 Bit, 1:1 primary prescale, 8:1 secondary prescale, CKE=1, MASTER mode
 */
#define PROPER_SPICON1 (0x0003 | 0x0520)
...
/* Set the input attenuator softpot VOTER IC1 (MCP4131) */
...
/* Initialize squelch values to default values */
...
void service_squelch(WORD diode,WORD sqpos,WORD noise,BOOL cal,BOOL wvf,BOOL iscaled) {
	...
	/* looptimer is used in the various state machines it. It just burns cycles down to 0. */
	if (looptimer) {
		looptimer--;
	}

	/* The tightest squelch setting has to be gte to hysteresis + 2 */
	if (sqpos < AppConfig.Hysteresis + 2 ) {
		sqpos = AppConfig.Hysteresis + 2 ;
	}
	 ...
	 * caldiode is the diode voltage stored in NVRAM during calibration
	 */
	 ...
	 * "CHUCK_SQUELCH"
	 */
	 /* average over the last 120ms including current noise */
	mavnoise32 = ((mavnoise32 * 31) + noise) >> 5;
	/* Take an average of the oldest 2 samples in the history buffer
	 * on arrival here noisehead points to oldest sample in 16 sample array
	 * this averages samples back 13 and 14 from now in buffer to see what the
	 * signal was like 50ms before the unkey??
	 */
	x = noisehead + 1;
	x &= (NHBSIZE - 1);
	noise50msago = noisehistory[x++];  /* Repaired this line to increment x rather than add 1 to noise 50ms ago */
	x &= (NHBSIZE - 1);
	...
	if (cal) {
		...
		switch (calstate) {
			case START:
				noise_gain = 0; /* Start calibration with the wiper of the MCP4131 at ground (PDW=PDB) */
				looptimer = 100; /* For debouncing */
				calstate = DEBOUNCE;
				break;

			case DEBOUNCE:
				/* When the looptimer (started at 100) hits 0, move on to SQSET */
				if (!looptimer) {
					/* Double check we are still in calibration mode */
					if (cal) {
						sqled = 0;
						calstate = SQSET;
					}
			...
			case SQSET:
				...
				/* This just flashes the LED as it is calibrating */
			 ...
			 */  			
			case TEST:
				/* Every 129th time we get here, check the noise level. */
				if (!looptimer) {
		...
		/* State machine */
		switch (sqstate) {
			/* Always start with the squelch closed */
			case CLOSED:
				sqled = 0;
				cor = 0;
				/* Check if signal (mavnoise32) is quieter (stronger) than
				 * lower hysteresis point, if it is, open the squelch. Otherwise,
				 * leave it closed. 
				 */
			...
			case OPEN:
				...
				sqled = 1;
				/* If the squelch is open, now check and see if the signal is getting
				 * noisy (larger value)
				 *
				 * This is the 2-stage squelch action.
				 * First, let's check and see if we had a strong signal, but now the current
				 * value (noise) is larger than the positive hysteresis (squelch close threshold),
				 * and close the squelch instantly.
				 */
				...
				/* This is the "normal" squelch action, just checking to see if the average signal
				 * level is greater (noisier) than the closure threshold.
				 */
